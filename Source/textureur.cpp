...
void Textureur::init(DescripteurTache& tache) {
    ...
    cacheBestCoupe = tache.cacheBestCoupe;
  ...
  if (cacheBestCoupe)
  {
    BestCoupeHBs = (int*) malloc(nb_bloc*nb_bloc*bloc_w*sizeof(int));
    BestCoupeGDs = (int*) malloc(nb_bloc*nb_bloc*bloc_h*sizeof(int));
    cachedCoupeHBs = (bool*) malloc(nb_bloc*nb_bloc*sizeof(bool));
    cachedCoupeGDs = (bool*) malloc(nb_bloc*nb_bloc*sizeof(bool));
    for (int i = 0; i < nb_bloc*nb_bloc; i++)
    {
        cachedCoupeHBs[i] = false;
        cachedCoupeGDs[i] = false;
    }
  }
        ...
        switch (voisinage) {
            case 0: {
                if (cacheBestBlock)
                {
                    ...
                    else // calculate and save
                    {
                        ...
                        if (cacheBestCoupe)
                        {
                            cachedCoupeHBs[vh*nb_bloc+i] = true;
                            for (int j = 0; j < bloc_w; j++)
                                BestCoupeHBs[(vh*nb_bloc + i)*bloc_w + j] = coupehb[j];
                        }
                    }
                    ...
                    {
                        coutGD = Coupe_GD(raccordeur, &table_blocs[vg], &table_blocs[i], coupegd); // calculate
                        coutGDs->set(vg, i, coutGD); // save
                        if (cacheBestCoupe)
                        {
                            cachedCoupeGDs[vg*nb_bloc+i] = true;
                            for (int j = 0; j < bloc_h; j++)
                                BestCoupeGDs[(vg*nb_bloc + i)*bloc_h + j] = coupegd[j];
                        }
                    }
                ...
                else
                {
                    ...
                    if (cacheBestCoupe  && !cachedCoupeHBs[vh*nb_bloc+i])
                    {
                        cachedCoupeHBs[vh*nb_bloc+i] = true;
                        for (int j = 0; j < bloc_w; j++)
                            BestCoupeHBs[(vh*nb_bloc + i)*bloc_w + j] = coupehb[j];
                    }
                    coutGD = Coupe_GD(raccordeur, &table_blocs[vg], &table_blocs[i], coupegd);
                    if (cacheBestCoupe && !cachedCoupeGDs[vg*nb_bloc+i])
                    {
                        cachedCoupeGDs[vg*nb_bloc+i] = true;
                        for (int j = 0; j < bloc_h; j++)
                            BestCoupeGDs[(vg*nb_bloc + i)*bloc_h + j] = coupegd[j];
            ...
            case 2:       // voisin haut seulement
            {
                if (cacheBestBlock)
                {
                    ...
                    {
                        cout = Coupe_HB(raccordeur, &table_blocs[vh], &table_blocs[i], coupehb); // calculate
    ...
    for (l = 0;l < res_verti; l++) { // indice ligne - progression par bloc
        for (c = 0; c < res_hori; c++) { // indice colonne
            ...
            if (c > 0) {
                ...
                {
                    for (int j = 0; j < bloc_h; j++)
                        coupe[j] = BestCoupeGDs[(res_bindex->get(c-1, l)*nb_bloc + ind)*bloc_h + j];
                }
                else
                {
                    Coupe_GD(raccordeur, &table_blocs[res_bindex->get(c-1, l)], &table_blocs[ind], coupe); // bug swap res_bindex->get(c-1, l), ind. b1 is previous block
                }
            ...
            if (l > 0) {
                ...
                {
                    for (int j = 0; j < bloc_w; j++)
                        coupe[j] = BestCoupeHBs[(res_bindex->get(c, l-1)*nb_bloc + ind)*bloc_w + j];
                }
                else
                {
                    Coupe_HB(raccordeur, &table_blocs[res_bindex->get(c, l-1)], &table_blocs[ind], coupe); // bug swap res_bindex->get(c, l-1), ind. b1 is previous block
                }

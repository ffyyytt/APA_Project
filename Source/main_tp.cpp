...
int main(int argc, char **argv){
    ...
    if (!strcmp(argv[argc-1], "--help")) {
        ...
        fprintf(stderr, " cache best block, defaut : %d\n", tache.cacheBestBlock);
        fprintf(stderr, " cache best cut, defaut : %d\n", tache.cacheBestCoupe);
        ...
        fprintf(stderr, " 6 : raccord avec coupe optimale (recursif naif), new cut array every call recursive\n");
        exit(0);
    }
    ...
    if (argc>=8) tache.cacheBestBlock = atoi(argv[7]);
    if (argc>=9) tache.cacheBestCoupe = atoi(argv[8]);
    if (argc>=10) tache.visualization = atoi(argv[9]);
    if (argc >10) fprintf(stderr, "on ignore les arguments Unix au dela du 6eme\n");
    ...
    switch(algo){
        ...
        case 5:
            ...
            break;
        case 6:
            tache.raccordeur = new RaccordeurRecursifNaif(true); // new cut array every call recursive
            break;
        default:
